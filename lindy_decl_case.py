...
def decl_on_case(fio: str, case: Case) -> str:
    ...
    if len(part_fio) == 3:  # проверяем на длину и обрабатываем только те что имеют длину 3 во всех остальных случаях просим просклонять самостоятельно
        ...
        return 'Проверьте количество слов в ФИО, должно быть 3 или 2 разделенных пробелами слова'

def extract_case_part_fio(fio:str,name_part:str):
    """
    Функция для извлечения просклоненной части ФИО
    """
    fio = fio.strip()  # очищаем строку от пробельных символов с начала и конца
    part_fio = fio.split()  # разбиваем по пробелам создавая список где [0] это Фамилия,[1]-Имя,[2]-Отчество

    if len(part_fio) == 3:  # проверяем на длину и обрабатываем только те что имеют длину 3 во всех остальных случаях просим просклонять самостоятельно
        if name_part == 'lastname':
            return part_fio[0] # возвращаем фамилию
        elif name_part == 'firstname':
            return part_fio[1] # возвращаем имя
        else:
            return part_fio[2] # возвращаем отчество
    elif len(part_fio) == 2:
        if name_part == 'lastname':
            return part_fio[0] # возвращаем фамилию
        elif name_part == 'firstname':
            ...
            return '' # возвращаем отчество


    else:
        return 'Проверьте количество слов в ФИО, должно быть 3 или 2 слова разделенных пробелами слова'
...
def declension_lst_fio_columns_by_case(df:pd.DataFrame,lst_name_columns:list)->pd.DataFrame:
    ...
    for fio_column in lst_name_columns:
        ...
        df[fio_column].fillna('', inplace=True)
        df[fio_column] = df[fio_column].apply(lambda x: x.strip())
        df[fio_column] = df[fio_column].apply(
            lambda x: x if x else '')  # Если пустая строка то заменяем на значение
        # создаем колонки
...
def declension_fio_by_case(df:pd.DataFrame,result_folder:str)->pd.DataFrame:
    ...
    try:
        ...
        if 'ФИО' in df.columns:
            fio_column = 'ФИО'
            df['ФИО'] = df['ФИО'].apply(lambda x:x.strip() if isinstance(x,str) else '')
            ...
            df['Фамилия'] = df['Фамилия'].apply(lambda x:x.strip() if isinstance(x,str) else '')
            df['Имя'] = df['Имя'].apply(lambda x:x.strip() if isinstance(x,str) else '')
            df['Отчество'] = df['Отчество'].apply(lambda x:x.strip() if isinstance(x,str) else '')
        ...
        # Склоняем по отдельности
        temp_df['Фамилия_род'] = temp_df['Родительный_падеж'].apply(lambda x:extract_case_part_fio(x,'lastname'))
        temp_df['Имя_род'] = temp_df['Родительный_падеж'].apply(lambda x:extract_case_part_fio(x,'firstname'))
        temp_df['Отчество_род'] = temp_df['Родительный_падеж'].apply(lambda x:extract_case_part_fio(x,'patronymic'))

        temp_df['Фамилия_дат'] = temp_df['Дательный_падеж'].apply(lambda x:extract_case_part_fio(x,'lastname'))
        temp_df['Имя_дат'] = temp_df['Дательный_падеж'].apply(lambda x:extract_case_part_fio(x,'firstname'))
        temp_df['Отчество_дат'] = temp_df['Дательный_падеж'].apply(lambda x:extract_case_part_fio(x,'patronymic'))

        temp_df['Фамилия_вин'] = temp_df['Винительный_падеж'].apply(lambda x:extract_case_part_fio(x,'lastname'))
        temp_df['Имя_вин'] = temp_df['Винительный_падеж'].apply(lambda x:extract_case_part_fio(x,'firstname'))
        temp_df['Отчество_вин'] = temp_df['Винительный_падеж'].apply(lambda x:extract_case_part_fio(x,'patronymic'))

        temp_df['Фамилия_твор'] = temp_df['Творительный_падеж'].apply(lambda x:extract_case_part_fio(x,'lastname'))
        temp_df['Имя_твор'] = temp_df['Творительный_падеж'].apply(lambda x:extract_case_part_fio(x,'firstname'))
        temp_df['Отчество_твор'] = temp_df['Творительный_падеж'].apply(lambda x:extract_case_part_fio(x,'patronymic'))

        temp_df['Фамилия_пред'] = temp_df['Предложный_падеж'].apply(lambda x:extract_case_part_fio(x,'lastname'))
        temp_df['Имя_пред'] = temp_df['Предложный_падеж'].apply(lambda x:extract_case_part_fio(x,'firstname'))
        temp_df['Отчество_пред'] = temp_df['Предложный_падеж'].apply(lambda x:extract_case_part_fio(x,'patronymic'))
        ...
        # Добавляем колонки с склоненными частями ФИО
        df.insert(index_fio_column + 30, 'Фамилия_род', temp_df['Фамилия_род'])
        df.insert(index_fio_column + 31, 'Имя_род', temp_df['Имя_род'])
        df.insert(index_fio_column + 32, 'Отчество_род', temp_df['Отчество_род'])

        df.insert(index_fio_column + 33, 'Фамилия_дат', temp_df['Фамилия_дат'])
        df.insert(index_fio_column + 34, 'Имя_дат', temp_df['Имя_дат'])
        df.insert(index_fio_column + 35, 'Отчество_дат', temp_df['Отчество_дат'])

        df.insert(index_fio_column + 36, 'Фамилия_вин', temp_df['Фамилия_вин'])
        df.insert(index_fio_column + 37, 'Имя_вин', temp_df['Имя_вин'])
        df.insert(index_fio_column + 38, 'Отчество_вин', temp_df['Отчество_вин'])

        df.insert(index_fio_column + 39, 'Фамилия_твор', temp_df['Фамилия_твор'])
        df.insert(index_fio_column + 40, 'Имя_твор', temp_df['Имя_твор'])
        df.insert(index_fio_column + 41, 'Отчество_твор', temp_df['Отчество_твор'])

        df.insert(index_fio_column + 42, 'Фамилия_пред', temp_df['Фамилия_пред'])
        df.insert(index_fio_column + 43, 'Имя_пред', temp_df['Имя_пред'])
        df.insert(index_fio_column + 44, 'Отчество_пред', temp_df['Отчество_пред'])
