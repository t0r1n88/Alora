...
def create_age_border(df:pd.DataFrame,current_date:str):
    """
    Функция для создания двух датафреймов разделенных по дате рождения на определенную дату больше 18 и меньше 18
    Parameters
    ----------
    df датафрейм с данными обучающихся
    current_date дата по которой будет определяться текущий возраст
    ...
    df['Текущий_возраст'] = df['Дата_рождения'].apply(lambda x: calculate_age(x, current_date))

    adult_df = df[df['Текущий_возраст']>=18]
    minor_df = df[df['Текущий_возраст']<=18]

    return adult_df,minor_df
