"""
Скрипт для подсчета количества зачисленных
"""
import pandas as pd
import openpyxl
import time
...
def generate_data_for_priem_enroll_yandex(data_file:str,end_folder:str):
    """
    ...
    lst_cols = ['ОУ', 'Код и наименование', 'База',
                ...
                'Всего заявлений', 'Зачислено Всего', 'Зачислено Целевые', 'Зачислено Профессионалитет',
                'Зачислено участники СВО', 'Зачислено дети участников СВО']
    ...
    for sheet in lst_sheets:
        ...
        # удаляем колонки относящиеся к Зачислено с сентября по октябрь
        ...
        # Находим последнюю колонку Всего для зачисленных
        ...
        out_temp_df = pd.concat([
            temp_df.iloc[:, :9],  # первые 9 колонок
            temp_df.iloc[:, last_idx:last_idx+5]  # последние 6 колонок
        ], axis=1)
    ...
    main_df['Код и наименование'] = main_df['Код и наименование'].replace({'23.02.06 Техническая эксплуатация подвижного состава железных дорог (вагоны)':'23.02.06 Техническая эксплуатация подвижного состава железных дорог',
                                                                           '23.02.06 Техническая эксплуатация подвижного состава железных дорог (локомотивы)':'23.02.06 Техническая эксплуатация подвижного состава железных дорог'})

    svod_df = pd.pivot_table(main_df,
                             values=['Всего заявлений', 'Зачислено Всего', 'Зачислено Целевые', 'Зачислено Профессионалитет',
                'Зачислено участники СВО', 'Зачислено дети участников СВО'],
                             index=['ОУ'],
                             aggfunc='sum',
                             fill_value=0)
    ...
    svod_df = svod_df.reindex(
        columns=['Всего заявлений', 'Зачислено Всего', 'Зачислено Целевые', 'Зачислено Профессионалитет',
                'Зачислено участники СВО', 'Зачислено дети участников СВО'])

    with pd.ExcelWriter(f'{end_folder}/ЗАЧИСЛЕНО Итоговый файл {current_date}.xlsx') as writer:
...
if __name__ == '__main__':
    ...
    generate_data_for_priem_enroll_yandex(main_data_file,main_end_result_folder)
