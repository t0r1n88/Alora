...
def calc_itog_part(value):
    """
    Функция для подсчета сдавших итоговую аттестацию
    :param value:
    :return:
    """
    if value >= 20.00:
...
def generate_svod_first_prof(list_student:str,estimation_file:str,lst_moodle:str,result_folder:str):
    ...
    try:
        ...
        df['сдавших итоговую аттестацию'] = df['Тест:Итоговая аттестация (Значение)'].apply(calc_itog_part)
        ...
        lst_for_lists = ['ФИО_представителя','Номер_телефона','Электронная_почта','ФИО',
                                      'Муниципалитет','Школа','Класс','сдавших первый модуль','сдавших второй модуль','сдавших третий модуль','сдавших итоговую аттестацию']
        ...
        # Аттестация
        create_lists(df[lst_for_lists].copy(),result_folder,1,'сдавших итоговую аттестацию','Сдавшие Итоговую аттестацию')
        create_lists(df[lst_for_lists].copy(),result_folder,0,'сдавших итоговую аттестацию','НЕ сдавшие Итоговую аттестацию')
        ...
        group_df = df.groupby(['Муниципалитет']).agg({'Тест:Тест 1.1. Речевая и логическая культура ведения делового разговора (Значение)':'count','сдавших первый модуль':'sum',
                                                      'сдавших второй модуль':'sum','сдавших третий модуль':'sum','сдавших итоговую аттестацию':'sum'}).fillna(0)
        ...
        # Аттестация
        group_df['% сдавших итоговую аттестацию'] = round((group_df['сдавших итоговую аттестацию'] / group_df['Записано на курс']) * 100, 1)
        ...
        group_df = group_df.reindex(columns=['Записано на курс','приступивших к обучению','% приступивших к обучению','НЕ приступивших к обучению','сдавших первый модуль','% сдавших первый модуль',
                                  'сдавших второй модуль','% сдавших второй модуль', 'сдавших третий модуль','% сдавших третий модуль','сдавших итоговую аттестацию','% сдавших итоговую аттестацию'])
        ...
        group_df.loc['Итого', '% сдавших итоговую аттестацию'] = round(
            (group_df.loc['Итого', 'сдавших итоговую аттестацию'] / group_df.loc['Итого', 'Записано на курс']) * 100, 1)
        ...
        df.insert(56,'Оценка Итоговая аттестация',df['Тест:Итоговая аттестация (Значение)'])



        df.drop(columns=['Номер_удостоверения','Рег_номер','Дата_выдачи','Номер_договор','Уровень_образования',
                         'Фамилия_в_дипломе','Серия_диплома','Номер_диплома','ФИО','Фамилия_y','Имя_y',
                         'Задание:Задание самостоятельной работы 1.2.1 (Значение)','Тест:Тест 1.1. Речевая и логическая культура ведения делового разговора (Значение)',
                         'Задание:Задание 2.1 (Значение)','Тест:Тест 3.3. Поисковые системы в сети Интернет: принципы работы и функциональные возможности (Значение)',
                         'Задание:Задание 2.2.1 (Значение)','Задание:Задание 2.2.2 (Значение)','Задание:Задание 2.2.3 (Значение)',
                         'Задание:Задание 3.6.1 (Значение)','Задание:Задание 3.6.2 (Значение)','Задание:Задание 3.6.3 (Значение)','Задание:Задание 3.2. (Значение)',
                         'Тест:Тест 3.7. Основы работы с базами данных (Значение)','Итоговая оценка за курс (Значение)','Тест:Итоговая аттестация (Значение)'],inplace=True)

        df.rename(columns={'Тест:Итоговая аттестация (Значение)':'Оценка Итоговая аттестация','сдавших первый модуль':'Первый модуль','сдавших второй модуль':'Второй модуль',
                           'сдавших третий модуль':'Третий модуль','сдавших итоговую аттестацию':'Итоговая аттестация'},inplace=True)


        df[['Первый модуль','Второй модуль','Третий модуль','Итоговая аттестация']] = df[['Первый модуль','Второй модуль','Третий модуль','Итоговая аттестация']].replace({0:'не сдан',1:'сдан'})

        df.to_excel(f'{result_folder}/Список с результатами.xlsx',index=False)
...
if __name__ == '__main__':
    main_list_student = 'data/ИТОГОВЫЙ список зарегистрировавшихся на курс 15 декабря.xlsx'
    main_estimation_file = 'data/Оценки.xlsx'
