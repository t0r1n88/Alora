"""
Скрипт для генерации сертификатов для викторины по чемпионату Профессионалы 2026
"""
"""
...
def clean_value(value):
    """
    Функция для обработки значений колонки
...
def generate_docs_from_template(name_file_template_doc, name_file_data_doc,name_column, name_type_file,path_to_end_folder_doc, name_value_column, mode_pdf,
    ...
    try:
        ...
        df['Фамилия'] = df['Фамилия'].apply(clean_value)
        df['Имя'] = df['Имя'].apply(clean_value)
        df['Отчество'] = df['Отчество'].apply(clean_value)


        df[name_column] = df['Фамилия'] + ' ' + df['Имя'] + ' ' + df['Отчество']
...
if __name__ == '__main__':
    ...
    name_file_data_doc_main = 'data/Викторина.xlsx'
    path_to_end_folder_doc_main = 'data/Результат'
