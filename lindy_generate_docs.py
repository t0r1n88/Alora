...
def generate_docs(dct_descr:dict,data_df:pd.DataFrame,source_folder:str,destination_folder:str,type_program:str,type_form:str,adult_df:pd.DataFrame,minor_df:pd.DataFrame):
    """
    ...
    :param adult_df: Датафрейм только с совершеннолетними
    :param minor_df: Датафрейм только с несовершеннолетними
    ...
    try:
        ...
        for key, value in dct_descr.items():
            minor_df[key] = value
        minor_lst_data_df = minor_df.copy()

        # для совершеннолетних
        for key, value in dct_descr.items():
            adult_df[key] = value
        adult_lst_data_df = adult_df.copy()
        ...
        minor_data = minor_df.to_dict('records')
        adult_data = adult_df.to_dict('records')
        ...
        for source_folder,dest_folder in dct_path.items():
            for file in os.listdir(source_folder):
                if 'Несовершеннолетние' not in source_folder and 'Совершеннолетние' not in source_folder:
                    ...
                    if 'Несовершеннолетние' in source_folder:
                        if file.endswith('.docx') and not file.startswith(
                            ...
                            if 'раздельный' in file.lower():
                                used_name_file = set()  # множество для уже использованных имен файлов
                                # Создаем в цикле документы
                                for idx, row in enumerate(minor_data):
                                    doc = DocxTemplate(f'{source_folder}/{file}')
                                    context = row
                                    # print(context)
                                    doc.render(context)
                                ...
                                doc = DocxTemplate(f'{source_folder}/{file}')
                                context = dict()
                                context['Итог'] = minor_lst_data_df.to_dict('records')
                                context.update(dct_descr)  # добавляем словарь с описанием программы
                                ...
                                doc.save(f'{dest_folder}/{name_file[:80]} {current_time}.docx')
                                used_name_file.add(name_file[:80])
                    elif 'Совершеннолетние' in source_folder:
                        if file.endswith('.docx') and not file.startswith(
                                '~$'):  # получаем только файлы docx и не временные
                            # определяем тип создаваемого документа
                            if 'раздельный' in file.lower():
                                used_name_file = set()  # множество для уже использованных имен файлов
                                # Создаем в цикле документы
                                for idx, row in enumerate(adult_data):
                                    doc = DocxTemplate(f'{source_folder}/{file}')
                                    context = row
                                    # print(context)
                                    doc.render(context)
                                ...
                                doc = DocxTemplate(f'{source_folder}/{file}')
                                context = dict()
                                context['Итог'] = adult_lst_data_df.to_dict('records')
                                context.update(dct_descr)  # добавляем словарь с описанием программы
